...
const {
  ...
  insertRowsViaCopy,
  collectMergedRows,
} = require('../src/postgres-select-merge')
const { rewriteQueryTableNameAst } = require('../src/select-order-rewrite')
const {
  resetLocalPostgresClientFactory,
  setLocalPostgresClientFactory,
} = require('../src/postgres-local')
...
describe('localhost load strategies', () => {
  ...
  it('uses unnest insert for medium row counts', async () => {
    ...
    const { mergeSelectResultsOnLocalPostgres } = require('../src/postgres-select-merge')
