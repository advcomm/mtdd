...
export function getMtddContext(): Record<string, unknown> | undefined
...
export const hooks: {
  ...
  onLookup: (
    req: { tid: string; hostCount: number; hostIndex?: number },
    next: () => Promise<number>,
  ) => Promise<number>
  onSelectHost: (
...
export function validateEnvDbHost(): unknown[]
export function validateLookupUrl(): string
export function parseListenNotifyStatement(text: string): unknown
